...
FEATS_DIM     = 180
K             = 2
ACTIV         = "ELU"                # activation for GAT (ELU recommended)
ALPHA         = 0.92
CUT           = 0
TAU           = 0.07
BETA          = 0.5
...
HIDDEN_DIM    = 256                  # hidden dimension for GAT and MLP
GAT_HEADS     = 1                    # number of attention heads per layer
GAT_DROPOUT   = 0.25                 # dropout inside GATConv
...
class GATEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, activ="ELU",
        ...
        # We'll concatenate heads for intermediate layers, then average for final?
        # Common practice: use multi-head for first layer, then single head for deeper.
        # Here we use same number of heads for all layers and sum/mean outputs.
        for i in range(num_layers):
            in_dim = input_dim if i == 0 else hidden_dim * heads   # because we concatenate heads
            out_dim = hidden_dim
            layers.append(GATConv(in_dim, out_dim, heads=heads, dropout=dropout,
                                  concat=True, negative_slope=0.2))
            bn_layers.append(nn.BatchNorm1d(out_dim * heads))
        self.convs = nn.ModuleList(layers)
        self.bns = nn.ModuleList(bn_layers)
        # Projection from concatenated multi-head output to final hidden_dim
...
class GATModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_clusters, device, activ="ELU",
                 ...
                 num_gat_layers=3, gat_heads=4, gat_dropout=0.25, T_struct=2.0):
        super().__init__()
        self.device = device
        ...
        self.online_encoder = GATEncoder(input_dim, hidden_dim, activ,
...
# ─── CLUSTERING METRICS (unchanged from ARMA version) ─────────────────────────
...
def print_clustering_summary(all_records, depth_label="GAT layers"):
    sep = "─" * 90
    print(f"\n{sep}")
...
def run_once(seed_offset=0, verbose=False, return_probs=False,
             n_eval_passes=30, num_gat_layers=NUM_GAT_LAYERS,
             gat_heads=GAT_HEADS, gat_dropout=GAT_DROPOUT):
    np.random.seed(42 + seed_offset)
    random.seed(42 + seed_offset)
    torch.manual_seed(42 + seed_offset)

    model = GATModel(FEATS_DIM, HIDDEN_DIM, K, DEVICE, ACTIV,
                     ema_decay=EMA_DECAY, cut=CUT, beta=BETA, tau=TAU,
                     num_gat_layers=num_gat_layers, gat_heads=gat_heads,
                     gat_dropout=gat_dropout, T_struct=T_STRUCT).to(DEVICE)
    ...
    # Unweighted metrics (for backwards compatibility)
    metrics_unw = (accuracy_score(y, yp),
    ...
    if return_probs:
        return metrics_unw, metrics_w, yp, ypp, logits_stack, logits_mean, model
...
print(f"\n{SEP}")
print(f"  PART A — FIRST RUN (GAT with {NUM_GAT_LAYERS} layers) – CN vs MCI")
...
metrics0_unw, metrics0_w, yp0, ypp0, logits_stack0, logits_mean0, model3 = run_once(
    ...
    n_eval_passes=N_EVAL_PASSES, num_gat_layers=NUM_GAT_LAYERS,
    gat_heads=GAT_HEADS, gat_dropout=GAT_DROPOUT
)
...
df_unc0.to_csv("gat_cluster_uncertainty_part_a.csv", index=False, float_format="%.6f")
print(f"\n  CSV → gat_cluster_uncertainty_part_a.csv  ({len(df_unc0)} subjects)")

# Clustering metrics for first run
clustering_results_A = evaluate_clustering_from_mc(
    model3, features_np, edge_index_np, y, yp0, logits_mean0, DEVICE,
    prefix="Part A — GAT, seed 0"
)

# ─── DEPTH ABLATION (vary number of GAT layers) ──────────────────────────────
print(f"\n{SEP}\n  PART B — DEPTH ABLATION (1 / 2 / 3 GAT layers)\n{SEP}")
ablation_results = {}
trained_models = {}
ablation_clustering = {1: [], 2: [], 3: []}

for n_layers in [1, 2, 3]:
    print(f"\n  ── Depth = {n_layers} GAT layer(s) ──")
    seed_records = []
    for seed in range(3):
        print(f"    Seed {seed} ... ", end="", flush=True)
        _, model_i = run_once(seed_offset=seed, num_gat_layers=n_layers,
...
# ─── 10-SEED EVALUATION (fixed 3 GAT layers) – using weighted metrics ────────
print(f"\n{SEP}\n  PART C — 10-SEED EVALUATION (3 GAT layers, CN vs MCI)\n{SEP}")
...
for i in range(10):
    m_unw, m_w, yp_i, ypp_i, logits_stack_i, logits_mean_i, model_i = run_once(
        ...
        n_eval_passes=N_EVAL_PASSES, num_gat_layers=3,
        gat_heads=GAT_HEADS, gat_dropout=GAT_DROPOUT
    )
...
sep100 = "─" * 100
print(f"\n{sep100}")
print(f"  10-RUN SUMMARY  (mean ± std)                         [GAT, 3 layers, CN vs MCI]")
...
print(f"\n  ── 10-SEED EVALUATION (3 GAT layers) ──")
print_clustering_summary(seed_clustering_records, depth_label="3 GAT layers — 10 seeds")

print(f"\n  ── DEPTH ABLATION (3 seeds per depth) ──")
for n_layers in [1, 2, 3]:
    print_clustering_summary(
        ablation_clustering[n_layers],
        depth_label=f"{n_layers} GAT layer{'s' if n_layers>1 else ''} — 3 seeds"
    )

# ─── FINAL SUMMARY (weighted) ─────────────────────────────────────────────────
print(f"\n{'='*60}")
print(f"  GAT — Final Summary (10 runs, 3 layers, {GAT_HEADS} heads, weighted metrics)")
print(f"{'='*60}")
print(f"  ACC (weighted)          : {np.mean(weighted_acc):.4f} ± {np.std(weighted_acc):.4f}")
print(f"  Weighted Precision      : {np.mean(weighted_prec):.4f} ± {np.std(weighted_prec):.4f}")
print(f"  Weighted Recall         : {np.mean(weighted_rec):.4f} ± {np.std(weighted_rec):.4f}")
...
print(f"\n  ✓ Saved cluster uncertainty CSV → gat_cluster_uncertainty_part_a.csv")
print(f"\n{'='*60}\n  ANALYSIS COMPLETE\n{'='*60}")
...
print(f"Entropy range: min={H_assign.min():.4f}, max={H_assign.max():.4f}")
print(f"Entropy mean: {H_assign.mean():.4f}")
print(f"Number of red points (entropy > 0.7): {(H_assign > 0.7).sum()}")
print(f"Number of blue points (entropy < 0.3): {(H_assign < 0.3).sum()}")

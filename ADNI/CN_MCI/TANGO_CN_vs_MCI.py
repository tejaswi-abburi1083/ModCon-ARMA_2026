...
def main():
    ...
    print("-" * 72)
    print("  TANGO BASELINE - CN vs MCI (weighted metrics)")
    print("  NOTE: Diagnostic labels used ONLY for post-hoc external validation.")
    print("-" * 72)
    print(f"\nFeatures: {X.shape}, Labels: {y.shape} (CN: {np.sum(y==0)}, MCI: {np.sum(y==1)})")
    ...
    print("\n" + "-" * 72)
    print("  10-RUN SUMMARY (mean +- std)                         [TANGO, CN vs MCI]")
    print("  NOTE: Diagnostic labels used only for post-hoc external validation.")
    print("-" * 72)

...
NUM_GAT_LAYERS = 1
HIDDEN_DIM = 256
GAT_HEADS = 2                      # number of attention heads per layer
...
# ─── MODEL: GAT ENCODER (replaces ARMA) ──────────────────────────────────────
...
class GATEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, activ="ELU",
                 num_layers=1, heads=2, dropout=0.25):
        super().__init__()
...
def print_uncertainty_report(df, ypp, y_true, yp, T_used, n_passes,
                              ...
                              csv_path="uncertainty_per_subject_CN_AD_GAT.csv"):
    sep  = "─" * 72
...
def layer_similarity_analysis(model, feats, ei, y, W0, prefix=""):
    ...
    for i, emb in enumerate(inters):
        results.append(compute_mad_metrics(emb, W0, y, name=f"GAT layer {i+1}"))
...
print(f"\n{SEP}\n  PART A — FIRST RUN (1 GAT layer) – CN vs AD\n{SEP}")
...
for n_layers in [1, 2, 3]:
    ...
    for seed in range(3):
        ...
        (acc_s, prec_s, rec_s, f1_s, ll_s), model_i = run_once(
            seed_offset=seed, num_gat_layers=n_layers,
...
if mad_depth[3][1] < mad_depth[3][0] * 0.5:
    ...
    print("    ✓ Gap preserved – GAT not overly smoothed")


# ─── PART E — 10-SEED EVALUATION WITH WEIGHTED METRICS ────────────────
print(f"\n{SEP}\n  PART E — 10-SEED EVALUATION (1 GAT layer, CN vs AD)\n{SEP}")
...
sep100 = "─" * 100
print(f"\n{sep100}")
print(f"  10-RUN SUMMARY  (mean ± std)                              [1 GAT layer, CN vs AD]")
print(f"{sep100}")
...
print(f"\n  ── F.1  10-SEED EVALUATION (1 GAT layer) ──")
print_clustering_summary(seed_clustering_records, depth_label="1 GAT layer — 10 seeds")
...
df_all.to_csv("gat_cn_vs_ad_10run_weighted_summary.csv", index=False, float_format="%.6f")
print(f"\n  ✓ Saved weighted summary → gat_cn_vs_ad_10run_weighted_summary.csv")
print(f"\n{'='*60}\n  ANALYSIS COMPLETE\n{'='*60}")

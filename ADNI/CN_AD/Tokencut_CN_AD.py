...
fa_ad_path = "/home/snu/Downloads/Histogram_AD_FA_20bin_updated.npy" # Changed from fa_mci_path

cn_data = np.load(fa_cn_path, allow_pickle=True)
ad_data = np.load(fa_ad_path, allow_pickle=True) # Changed from mci_data

X = np.vstack([cn_data, ad_data]).astype(np.float32) # Changed mci_data to ad_data
y = np.hstack([
    np.zeros(len(cn_data), dtype=np.int64),
    np.ones(len(ad_data), dtype=np.int64) # Changed mci_data to ad_data
])
...
print(f"Features: {X.shape}, Labels: {y.shape} (CN: {np.sum(y==0)}, AD: {np.sum(y==1)})") # Changed MCI to AD
...
all_metrics = {
    ...
    "recall_weighted": [],
    "f1_weighted": [],
    "f1_ad": [], # Changed f1_mci to f1_ad
    "nmi": [], "ari": [], "ami": [],
    "silhouette": [], "davies_bouldin": [], "log_loss": []
}

print("═" * 72)
print("  TOKENCUT BASELINE – CN vs AD (weighted metrics for fair comparison)") # Changed MCI to AD
print("  NOTE: Diagnostic labels used ONLY for post-hoc external validation.")
print("═" * 72)

print(f"\n  {'Run':>4}  {'Acc':>7}  {'Prec_w':>8}  {'Rec_w':>8}  {'F1_w':>8}  " \
      f"{'F1_AD':>8}  {'NMI':>7}  {'ARI':>7}  {'LogLoss':>9}") # Changed F1_MCI to F1_AD
print("  " + "─" * 95)
...
for run in range(num_runs):
    ...
    f1_ad = f1_score(y_run, y_pred, pos_label=1, zero_division=0) # Changed f1_mci to f1_ad
    ...
    all_metrics["f1_ad"].append(f1_ad) # Changed f1_mci to f1_ad
    ...
    print(f"  {run+1:>4}  {acc:>7.4f}  {prec_w:>8.4f}  {rec_w:>8.4f}  {f1_w:>8.4f}  " \
          f"{f1_ad:>8.4f}  {nmi:>7.4f}  {ari:>7.4f}  {logloss:>9.6f}") # Changed f1_mci to f1_ad
...
print("  Metrics: Accuracy, Weighted Precision, Weighted Recall, Weighted F1, " \
      "F1(AD), NMI, ARI, AMI, Silhouette, Davies\u2011Bouldin") # Changed F1(MCI) to F1(AD)
print("═" * 72)

manuscript_metrics = [
    "accuracy", "prec_weighted", "recall_weighted", "f1_weighted", "f1_ad", # Changed f1_mci to f1_ad
    "nmi", "ari", "ami", "silhouette", "davies_bouldin"
]
manuscript_labels = [
    "Accuracy", "Prec (weighted)", "Recall (weighted)", "F1 (weighted)", "F1 (AD)", # Changed F1 (MCI) to F1 (AD)
...
ext_metrics = ["accuracy", "prec_weighted", "recall_weighted", "f1_weighted", "f1_ad", "nmi", "ari", "ami", "log_loss"] # Changed f1_mci to f1_ad
...
print(f"  F1 (AD)                 : {np.mean(all_metrics['f1_ad']):.4f} \u00b1 {np.std(all_metrics['f1_ad']):.4f}") # Changed F1 (MCI) to F1 (AD), f1_mci to f1_ad

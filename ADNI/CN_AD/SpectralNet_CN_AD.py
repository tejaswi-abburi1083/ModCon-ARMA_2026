...
# ============================================================================
# All other classes and functions are identical to CN vs MCI version
# (SiameseNet, SpectralNet, contrastive_loss, spectral_loss, etc.)
# ============================================================================
...
print(f"\n{SEP}")
print(f"  SpectralNet (Siamese + SpectralNet + KMeans) – CN vs AD")
...
for run in range(N_RUNS):
    ...
    # Build pairwise dataset (same as before)
    pairs, pair_labels = [], []
    ...
    # Compute deterministic embeddings and K‑means centres
    spectral.eval()
    with torch.no_grad():
...
df_unc.to_csv("spectralnet_cn_vs_ad_uncertainty_run1.csv", index=False, float_format="%.6f")
print(f"\n  CSV → spectralnet_cn_vs_ad_uncertainty_run1.csv  ({len(df_unc)} subjects)")
...
# 10‑run summary
print(f"\n{sep100}")
print(f"  10-RUN SUMMARY  (mean ± std)                [SpectralNet, CN vs AD]")
...
count_labels = [
    ...
    ("FN  (AD→C0)", "AD assigned to Cluster 0"),
    ("TP  (AD→C1)", "AD assigned to Cluster 1"),
]
...
print_clustering_summary(seed_clustering_records, label="SpectralNet — 10 runs (CN vs AD)")
...
print(f"\n{'='*60}")
print(f"  SpectralNet — Final Summary ({N_RUNS} runs, weighted metrics) – CN vs AD")
print(f"{'='*60}")
...
df_summary.to_csv("spectralnet_cn_vs_ad_10run_summary.csv", index=False, float_format="%.6f")
print(f"\n  ✓ Saved → spectralnet_cn_vs_ad_10run_summary.csv")
print(f"\n{'='*60}\n  ANALYSIS COMPLETE\n{'='*60}")

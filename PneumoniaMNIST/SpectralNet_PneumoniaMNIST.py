...
print(f"\n{SEP}")
print(f"  SpectralNet (Siamese + SpectralNet + KMeans) – PneumoniaMNIST")
...
df_unc.to_csv("spectralnet_pneumoniamnist_uncertainty_run1.csv", index=False, float_format="%.6f")
print(f"\n  CSV → spectralnet_pneumoniamnist_uncertainty_run1.csv  ({len(df_unc)} subjects)")
...
print(f"\n{sep100}")
print(f"  10-RUN SUMMARY  (mean ± std)                [SpectralNet, PneumoniaMNIST]")
...
print(f"  {sep100}")
count_labels = [
    ("TN  (Normal→C0)", "Normal assigned to Cluster 0"),
    ("FP  (Normal→C1)", "Normal assigned to Cluster 1"),
    ("FN  (Pneumonia→C0)", "Pneumonia assigned to Cluster 0"),
    ("TP  (Pneumonia→C1)", "Pneumonia assigned to Cluster 1"),
]
...
print_clustering_summary(seed_clustering_records, label="SpectralNet — PneumoniaMNIST (RadioDINO)")
...
df_summary.to_csv("spectralnet_pneumoniamnist_10run_summary.csv", index=False, float_format="%.6f")
print(f"\n  ✓ Saved → spectralnet_pneumoniamnist_10run_summary.csv")
print(f"\n{'='*60}\n  ANALYSIS COMPLETE\n{'='*60}")

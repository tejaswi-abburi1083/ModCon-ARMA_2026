...
K = 2
ACTIV = "ELU"
ALPHA = 0.92                     # graph threshold (higher for PneumoniaMNIST)
...
NUM_EPOCHS = 2000                # 2000 epochs (as requested)
N_EVAL_PASSES = 30               # MC passes for uncertainty
NUM_RUNS = 10                    # number of independent runs
NUM_ARMA_LAYERS = 1              # depth of ARMAConv stack (3 layers)
...
# ═══════════════════════════════════════════════════════════════════════════
#  MODEL DEFINITIONS (ARMA with 3 stacked layers + contrastive loss)
# ═══════════════════════════════════════════════════════════════════════════
...
class ARMAEncoder(torch.nn.Module):
    """Stack of NUM_ARMA_LAYERS ARMAConv layers with batch norm, dropout, and final projection."""
    def __init__(self, input_dim, hidden_dim, device, activ="ELU", num_arma_layers=3):
        super(ARMAEncoder, self).__init__()
        self.device = device
        self.num_arma_layers = num_arma_layers
        activations = {
    ...
    def forward(self, data):
        ...
        for i in range(self.num_arma_layers):
...
class ARMA(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_clusters, device, activ, moving_average_decay=0.5, cut=True,
        ...
        self.online_encoder = ARMAEncoder(input_dim, hidden_dim, device, activ, num_arma_layers=num_arma_layers)
...
def train_once(seed, verbose=True):
    ...
    # Build base graph (full) with ALPHA = 0.92
    W0 = create_adj(X, CUT, ALPHA)
    edge_index_np, _ = edge_index_from_dense(W0)
    A1 = torch.from_numpy(W0).float().to(DEVICE)

    model = ARMA(FEATS_DIM, 256, K, DEVICE, ACTIV, moving_average_decay=EMA_DECAY,
                 cut=CUT, num_arma_layers=NUM_ARMA_LAYERS).to(DEVICE)
...
print("═" * 72)
print("  ARMA (original design, 3 ARMA layers, 2000 epochs) – PneumoniaMNIST (RadioDINO)")
...
print("\n" + "═" * 72)
print("  10-RUN SUMMARY (mean ± std)                         [ARMA (original, 3 layers, 2000 epochs), PneumoniaMNIST]")
print("  NOTE: Diagnostic labels used only for post-hoc external validation.")
print("═" * 72)
...
if last_logits_stack is not None:
    ...
    df.to_csv("arma_original_pneumoniamnist_uncertainty_last_run.csv", index=False, float_format="%.6f")
    print(f"\n  ✓ Saved per-subject uncertainty to arma_original_pneumoniamnist_uncertainty_last_run.csv")
...
df_all.to_csv("arma_original_pneumoniamnist_10run_weighted_summary.csv", index=False, float_format="%.6f")
print(f"\n  ✓ Saved 10‑run weighted summary → arma_original_pneumoniamnist_10run_weighted_summary.csv")

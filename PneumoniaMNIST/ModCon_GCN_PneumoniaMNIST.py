...
class GCNEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, device, activ="ELU", num_gcn_layers=3):
        super().__init__()
        self.device = device
...
def run_once(seed_offset=0, verbose=False, return_probs=False,
             n_eval_passes=30, num_gcn_layers=3):
    np.random.seed(42 + seed_offset)
    random.seed(42 + seed_offset)
    torch.manual_seed(42 + seed_offset)

    model = GCNModel(FEATS_DIM, 256, K, DEVICE, ACTIV,
...
print(f"\n{SEP}")
print(f"  PART A — FIRST RUN (3 GCN layers) – PneumoniaMNIST (RadioDINO {RADIODINO_MODEL})")
...
df_unc0.to_csv("cluster_uncertainty_pneumonia_gcn.csv", index=False, float_format="%.6f")
print(f"\n  CSV → cluster_uncertainty_pneumonia_gcn.csv  ({len(df_unc0)} subjects)")
...
print("  Note: GCN does not have ARMA's band‑pass filter; we skip MADGap over‑smoothing analysis.")
print("  For GCN, deeper layers may cause over‑smoothing – we evaluate clustering metrics directly.\n")
...
# Skip MADGap analysis (not meaningful for GCN); we just print simple summary
...
print(f"\n{SEP}\n  PART D — DIAGNOSIS SUMMARY (PneumoniaMNIST, GCN)\n{SEP}")
...
# =============================================================================
print(f"\n{SEP}\n  PART E — 10-SEED EVALUATION (3 GCN layers, PneumoniaMNIST)\n{SEP}")
...
sep100 = "─" * 100
print(f"\n{sep100}")
print(f"  10-RUN SUMMARY  (mean ± std)              [3 GCN layers, PneumoniaMNIST / RadioDINO]")
...
print(f"\n  ── F.1  10-SEED EVALUATION (3 GCN layers) ──")
print_clustering_summary(seed_clustering_records,
                         depth_label="3 GCN layers — 10 seeds (PneumoniaMNIST, RadioDINO)")

print(f"\n  ── F.2  DEPTH ABLATION (3 seeds per depth) ──")
for n_layers in [1, 2, 3]:
    print_clustering_summary(
        ablation_clustering[n_layers],
        depth_label=(f"{n_layers} GCN layer{'s' if n_layers > 1 else ' '} — 3 seeds (PneumoniaMNIST, RadioDINO)")
    )
...
print(f"    Rising ARI/NMI/AMI with depth → deeper GCN layers may recover more label structure,")
print(f"    but too many layers can cause over-smoothing and degrade performance.")
print(f"    (GCN is not a band‑pass filter like ARMA.)\n")

# =============================================================================
# PART G — BIOLOGICAL / IMAGE FEATURE INTERPRETATION (unchanged, but skip MADGap)
# =============================================================================
print(f"\n{SEP}\n  PART G — BIOLOGICAL INTERPRETATION OF DISCOVERED CLUSTERS (GCN)\n{SEP}")
...
# Use the same cosine distance matrix function (without ARMA-specific parts)
...
print(f"    structure in chest X‑ray images. The GCN graph network organises")
...
df_paper.to_csv("cluster_pneumonia_characterization_gcn.csv", index=False)
print(f"\n  ✓ Saved to: cluster_pneumonia_characterization_gcn.csv")
df_unc0.to_csv("cluster_uncertainty_pneumonia_gcn.csv", index=False, float_format="%.6f")
print(f"  ✓ Saved uncertainty to: cluster_uncertainty_pneumonia_gcn.csv")

...
# Subsample: up to 2000 per class
...
print(f"Features: {features_np.shape}, Labels: {y.shape} "
      f"(Normal: {np.sum(y==0)}, Pneumonia: {np.sum(y==1)})")
    ...
    def __init__(self, input_dim, hidden_dim, device, activ="ELU",
        ...
        def _arma(i, o):
            return ARMAConv(i, o, num_stacks=num_stacks, num_layers=num_layers,
                            act=self.act, shared_weights=True, dropout=0.3)
...
print(f"\n{SEP}")
print(f"  PART A — FIRST RUN (3 ARMA layers) – PneumoniaMNIST (RadioDINO {RADIODINO_MODEL})")
...
print(f"  Logit diff (Pneumonia−Normal): mean={diff.mean():.2f}  "
...
df_unc0.to_csv("cluster_uncertainty_pneumonia.csv", index=False, float_format="%.6f")
print(f"\n  CSV → cluster_uncertainty_pneumonia.csv  ({len(df_unc0)} subjects)")
...
layer_similarity_analysis(model3, features_np, edge_index_np, y, W0,
                          prefix="3-layer ARMA (PneumoniaMNIST / RadioDINO)")
...
print(f"\n{SEP}\n  PART D — DIAGNOSIS SUMMARY (PneumoniaMNIST)\n{SEP}")
...
# =============================================================================
print(f"\n{SEP}\n  PART E — 10-SEED EVALUATION (3 ARMA layers, PneumoniaMNIST)\n{SEP}")
...
sep100 = "─" * 100
print(f"\n{sep100}")
print(f"  10-RUN SUMMARY  (mean ± std)              [3 ARMA layers, PneumoniaMNIST / RadioDINO]")
...
count_labels = [
    ("TN (Normal→C0)",   "Normal assigned to Cluster 0 — post-hoc"),
    ("FP (Normal→C1)",   "Normal assigned to Cluster 1 — post-hoc"),
    ("FN (Pneumo→C0)",   "Pneumonia assigned to Cluster 0 — post-hoc"),
    ("TP (Pneumo→C1)",   "Pneumonia assigned to Cluster 1 — post-hoc"),
]
...
print_clustering_summary(seed_clustering_records,
                         depth_label="3 ARMA layers — 10 seeds (PneumoniaMNIST, RadioDINO)")

print(f"\n  ── F.2  DEPTH ABLATION (3 seeds per depth) ──")
for n_layers in [1, 2, 3]:
    print_clustering_summary(
        ablation_clustering[n_layers],
        depth_label=(f"{n_layers} ARMA layer{'s' if n_layers > 1 else ' '} — 3 seeds (PneumoniaMNIST, RadioDINO)")
    )
...
print("    • Label distribution (Normal vs Pneumonia)")
...
print(f"    Normal   (n={(y==0).sum()})      {cm_diag[0,0]:>6}       {cm_diag[0,1]:>6}")
print(f"    Pneumonia(n={(y==1).sum()})      {cm_diag[1,0]:>6}       {cm_diag[1,1]:>6}")

pct_norm_in_c0 = (y[cluster0_mask] == 0).mean() * 100
pct_pneumo_in_c1 = (y[cluster1_mask] == 1).mean() * 100
agreement = (cluster_assignments == y).mean() * 100
print(f"\n  Cluster composition:")
print(f"    Cluster 0: {pct_norm_in_c0:.1f}% Normal, {100-pct_norm_in_c0:.1f}% Pneumonia")
print(f"    Cluster 1: {100-pct_pneumo_in_c1:.1f}% Normal, {pct_pneumo_in_c1:.1f}% Pneumonia")
...
print(f"  {'Normal %':<40} {f'{pct_norm_in_c0:.1f}%':>20} {f'{100-pct_pneumo_in_c1:.1f}%':>20} {'(ref. only)':>15}")
print(f"  {'Pneumonia %':<40} {f'{100-pct_norm_in_c0:.1f}%':>20} {f'{pct_pneumo_in_c1:.1f}%':>20} {'(ref. only)':>15}")
...
if agreement > 70:
    ...
    print(f"    Cluster 0 is predominantly Normal ({pct_norm_in_c0:.1f}%).")
    print(f"    Cluster 1 is predominantly Pneumonia ({pct_pneumo_in_c1:.1f}%).")
else:
...
print(f"    structure in chest X‑ray images. The ARMA graph network organises")
print(f"    these features into clusters that align substantially with clinically")
print(f"    defined Normal vs Pneumonia categories, without ever observing diagnostic")
print(f"    labels during training.")
print(f"\n  Limitations:")
...
paper_results = {
    "Metric": [
        ...
        "Normal % in Cluster 0", "Pneumonia % in Cluster 1",
    ...
    "Value": [
        ...
        f"{pct_norm_in_c0:.1f}%", f"{pct_pneumo_in_c1:.1f}%",
        f"{agreement:.1f}%",
...
df_paper.to_csv("cluster_pneumonia_characterization_radiodino.csv", index=False)
print(f"\n  ✓ Saved to: cluster_pneumonia_characterization_radiodino.csv")
df_unc0.to_csv("cluster_uncertainty_pneumonia_radiodino.csv", index=False, float_format="%.6f")
print(f"  ✓ Saved uncertainty to: cluster_uncertainty_pneumonia_radiodino.csv")
...
# =============================================================================
# PART H — t-SNE VISUALISATION (PneumoniaMNIST)
# =============================================================================
print(f"\n{SEP}\n  PART H — t-SNE: Input Features vs. ModCon-ARMA Embeddings (PneumoniaMNIST)\n{SEP}")
...
# True labels: y (0=Normal, 1=Pneumonia)
normal_mask = (y == 0)
pneumonia_mask = (y == 1)
...
ax1.scatter(tsne_input_results[normal_mask, 0],
            ...
            c='#2E86AB', label='Normal (True)', alpha=0.7, s=40,
            edgecolors='white', linewidth=0.5)
ax1.scatter(tsne_input_results[pneumonia_mask, 0],
            tsne_input_results[pneumonia_mask, 1],
            c='#D62728', label='Pneumonia (True)', alpha=0.7, s=40,
            edgecolors='white', linewidth=0.5)
...
ax2.scatter(tsne_hidden_results[cluster0_mask, 0],
            ...
            c='#2E86AB', label='Cluster 0 (Normal-dominant)', alpha=0.7, s=40,
            edgecolors='white', linewidth=0.5)
ax2.scatter(tsne_hidden_results[cluster1_mask, 0],
            tsne_hidden_results[cluster1_mask, 1],
            c='#D62728', label='Cluster 1 (Pneumonia-dominant)', alpha=0.7, s=40,
            edgecolors='white', linewidth=0.5)
...
plt.suptitle(f't-SNE Comparison (PneumoniaMNIST, perplexity={best_perp})', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('tsne_fig1_main_comparison_PneumoniaMNIST.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.show()
print("  ✓ Saved: tsne_fig1_main_comparison_PneumoniaMNIST.png")
...
ax.scatter(tsne_hidden_results[normal_mask, 0],
           ...
           c='#2E86AB', label='Normal (True label)', alpha=0.7, s=50,
           edgecolors='white', linewidth=0.5)
ax.scatter(tsne_hidden_results[pneumonia_mask, 0],
           tsne_hidden_results[pneumonia_mask, 1],
           c='#D62728', label='Pneumonia (True label)', alpha=0.7, s=50,
           edgecolors='white', linewidth=0.5)
...
plt.tight_layout()
plt.savefig('tsne_fig2_true_labels_validation_PneumoniaMNIST.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.show()
print("  ✓ Saved: tsne_fig2_true_labels_validation_PneumoniaMNIST.png")
...
plt.tight_layout()
plt.savefig('tsne_fig3_uncertainty_heatmap_PneumoniaMNIST.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.show()
print("  ✓ Saved: tsne_fig3_uncertainty_heatmap_PneumoniaMNIST.png")
...
if misclassified_mask.sum() > 0:
    ...
    plt.tight_layout()
    plt.savefig('tsne_fig4_misclassified_PneumoniaMNIST.png', dpi=300, bbox_inches='tight', facecolor='white')
    plt.show()
    print("  ✓ Saved: tsne_fig4_misclassified_PneumoniaMNIST.png")

print("\n  ✅ All t-SNE figures saved for PneumoniaMNIST.")
...
# =============================================================================
# PART H — t-SNE VISUALIZATION OF CLUSTERS (PneumoniaMNIST / RadioDINO)
# =============================================================================
print(f"\n{SEP}\n  PART H — t-SNE VISUALIZATION OF DISCOVERED CLUSTERS (PneumoniaMNIST)\n{SEP}")
...
cluster0_mask = (yp0 == 0)   # Normal‑dominant cluster? (depending on alignment)
cluster1_mask = (yp0 == 1)   # Pneumonia‑dominant cluster?
normal_mask = (y == 0)       # Normal ground truth
pneumonia_mask = (y == 1)    # Pneumonia ground truth
...
ax.scatter(tsne_results[cluster0_mask, 0], tsne_results[cluster0_mask, 1],
           c='#2E86AB', label='Cluster 0 (Normal‑dominant)',
...
ax.scatter(tsne_results[cluster1_mask, 0], tsne_results[cluster1_mask, 1],
           c='#A23B72', label='Cluster 1 (Pneumonia‑dominant)',
           alpha=0.7, s=50, edgecolors='white', linewidth=0.5, zorder=2)
...
ax.set_title('t-SNE: ARMA Embeddings (PneumoniaMNIST)\nColored by Cluster Assignment',
...
plt.tight_layout()
plt.savefig('tsne_by_cluster_assignment_PneumoniaMNIST.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.show()
print("  ✓ Saved: tsne_by_cluster_assignment_PneumoniaMNIST.png")
...
ax.scatter(tsne_results[pneumonia_mask, 0], tsne_results[pneumonia_mask, 1],
           c='#A23B72', label='Pneumonia (True label)',
           alpha=0.7, s=50, edgecolors='white', linewidth=0.5)
ax.scatter(tsne_results[normal_mask, 0], tsne_results[normal_mask, 1],
           c='#2E86AB', label='Normal (True label)',
           alpha=0.7, s=50, edgecolors='white', linewidth=0.5)
...
plt.tight_layout()
plt.savefig('tsne_by_true_diagnosis_PneumoniaMNIST.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.show()
print("  ✓ Saved: tsne_by_true_diagnosis_PneumoniaMNIST.png")
...
plt.tight_layout()
plt.savefig('tsne_uncertainty_heatmap_PneumoniaMNIST.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.show()
print("  ✓ Saved: tsne_uncertainty_heatmap_PneumoniaMNIST.png")
...
ax = axes[1]
ax.scatter(tsne_results[pneumonia_mask, 0], tsne_results[pneumonia_mask, 1],
           c='#A23B72', label='Pneumonia', alpha=0.7, s=40, edgecolors='white', linewidth=0.5)
ax.scatter(tsne_results[normal_mask, 0], tsne_results[normal_mask, 1],
           c='#2E86AB', label='Normal', alpha=0.7, s=40, edgecolors='white', linewidth=0.5)
...
plt.suptitle(f't-SNE Visualization (PneumoniaMNIST, perplexity={best_perp})', fontsize=12, fontweight='bold')
plt.tight_layout()
plt.savefig('tsne_comparison_PneumoniaMNIST.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.show()
print("  ✓ Saved: tsne_comparison_PneumoniaMNIST.png")
...
ax.set_title('Distribution of Cluster Assignment Entropy (PneumoniaMNIST)', fontsize=12, fontweight='bold')
...
plt.tight_layout()
plt.savefig('entropy_distribution_PneumoniaMNIST.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.show()
print("  ✓ Saved: entropy_distribution_PneumoniaMNIST.png")
...
sep72 = "─" * 72
print(f"\n{sep72}")
print("  CLUSTER SEPARATION METRICS (PneumoniaMNIST - from hidden embeddings)")
print(f"{sep72}")

print(f"\n  Intra-cluster distance (Cluster 0 - Normal‑dominant): {intra0:.4f}")
print(f"  Intra-cluster distance (Cluster 1 - Pneumonia‑dominant):    {intra1:.4f}")
...
print(f"\n{sep72}")
print("  t-SNE visualization complete (PneumoniaMNIST)")
print(f"{sep72}\n")
...
# =============================================================================
# PART H — GRAD‑CAM FOR CLUSTER MEMBERSHIP (PneumoniaMNIST)
# =============================================================================
print("\n" + "═" * 72)
print("  Grad‑CAM for Cluster Membership Prediction (PneumoniaMNIST)")
print("═" * 72)
...
# ----- 1. Extract subsampled images and align with cluster assignments -----
# final_dataset contains the subsampled images (order from combined_indices)
# yp0 are the cluster assignments after the global permutation (seed=42)
# We need to bring images into the same permuted order.
N_sub = len(final_dataset)
subsampled_images = torch.stack([final_dataset[i][0] for i in range(N_sub)])  # (N_sub, 3, 224, 224)
subsampled_labels = torch.tensor([final_dataset[i][1] for i in range(N_sub)])  # true diagnosis

# Shuffle with the same seed used for features (seed=42)
np.random.seed(42)
perm_imgs = np.random.permutation(N_sub)
subsampled_images = subsampled_images[perm_imgs]
subsampled_labels = subsampled_labels[perm_imgs]

# Now subsampled_images, subsampled_labels, yp0, and y are all aligned.
cluster_targets = torch.tensor(yp0, dtype=torch.long)      # cluster assignment (0 or 1)
true_labels = torch.tensor(y, dtype=torch.long)            # true diagnosis (0=Normal, 1=Pneumonia)

print(f"  Subsampled images: {subsampled_images.shape}")
print(f"  Cluster targets: {cluster_targets.shape}  (C0={(cluster_targets==0).sum().item()}, C1={(cluster_targets==1).sum().item()})")
print(f"  True labels: {true_labels.shape}  (Normal={(true_labels==0).sum().item()}, Pneumonia={(true_labels==1).sum().item()})")
...
train_images = subsampled_images[train_idx]
train_clusters = cluster_targets[train_idx]
val_images   = subsampled_images[val_idx]
val_clusters = cluster_targets[val_idx]
test_images  = subsampled_images[test_idx]
test_clusters = cluster_targets[test_idx]

train_dataset = TensorDataset(train_images, train_clusters)
val_dataset   = TensorDataset(val_images, val_clusters)
test_dataset  = TensorDataset(test_images, test_clusters)
...
print(f"  Test cluster distribution: C0={(test_clusters==0).sum().item()}, C1={(test_clusters==1).sum().item()}")

# ----- 3. Train ResNet18 classifier to predict cluster membership -----
device = DEVICE
...
print("\nTraining ResNet18 to predict ARMA cluster assignments (yp0)...")
...
print(f"  Saliency maps: Cluster 0 = {len(saliency_c0)}, Cluster 1 = {len(saliency_c1)}")

# ----- 5. Quantitative metrics per saliency map -----
def compute_saliency_metrics(heatmap):
    ...
    total_act = heatmap.sum() + 1e-8
    weighted_center_x = (heatmap * x_coords).sum() / total_act
    weighted_center_y = (heatmap * y_coords).sum() / total_act
    spread = np.sqrt(((heatmap * ((x_coords - weighted_center_x)**2 + (y_coords - weighted_center_y)**2)).sum() / total_act))
...
print("─" * 60)
print(f"{'Metric':<20} {'Cluster 0 mean±std':>22} {'Cluster 1 mean±std':>22} {'p-value':>10}")
print("─" * 60)
...
# ----- 6. Average saliency maps and difference map -----
...
# ----- 7. Visualisation with average maps and bar chart -----
fig, axes = plt.subplots(1, 4, figsize=(16, 5))
fig.suptitle(f"Quantitative Saliency Analysis (PneumoniaMNIST, acc={cluster_acc:.3f})", fontsize=14)

im0 = axes[0].imshow(avg_saliency_c0, cmap='jet', vmin=0, vmax=1)
axes[0].set_title(f"Avg Saliency - Cluster 0 (n={len(saliency_c0)})")
axes[0].axis('off')
plt.colorbar(im0, ax=axes[0], fraction=0.046)

im1 = axes[1].imshow(avg_saliency_c1, cmap='jet', vmin=0, vmax=1)
axes[1].set_title(f"Avg Saliency - Cluster 1 (n={len(saliency_c1)})")
axes[1].axis('off')
plt.colorbar(im1, ax=axes[1], fraction=0.046)

im2 = axes[2].imshow(diff_map, cmap='RdBu_r', vmin=-0.3, vmax=0.3)
...
plt.tight_layout()
plt.savefig("gradcam_quantitative_analysis_pneumonia.png", dpi=150)
plt.show()
print("\n  ✓ Saved quantitative analysis to 'gradcam_quantitative_analysis_pneumonia.png'")

# ----- 8. Representative examples -----
# Select 6 samples from each cluster (random)
test_clusters_np = test_clusters.numpy()
cluster0_test_ids = np.where(test_clusters_np == 0)[0]
cluster1_test_ids = np.where(test_clusters_np == 1)[0]
...
fig2.suptitle(f"Representative Grad-CAM Examples (PneumoniaMNIST, acc={cluster_acc:.3f})", fontsize=14)
...
for idx, sample_pos in enumerate(samples_cluster0):
    ...
    axes2[0, idx].set_title(f"True Cluster: {true_cluster}")
...
for idx, sample_pos in enumerate(samples_cluster1):
    ...
    axes2[1, idx].set_title(f"True Cluster: {true_cluster}")
    axes2[1, idx].axis('off')

plt.tight_layout()
plt.savefig("gradcam_representative_examples_pneumonia.png", dpi=150)
plt.show()
print("  ✓ Saved representative examples to 'gradcam_representative_examples_pneumonia.png'")

# ----- 9. Summary statistics -----
print("\n" + "═" * 60)
print("  SUMMARY STATISTICS (PneumoniaMNIST)")
print("═" * 60)
...
print(f"    unsupervised cluster assignments from raw chest X‑ray images, indicating that")
print(f"    the discovered clusters correspond to visually separable patterns.")
print(f"    Quantitative saliency metrics provide statistical evidence for differences")
print(f"    in attention patterns between clusters (see figures).")
print("\n  ✓ All outputs saved as *_pneumonia.png")
...
"""
ARMA Model – Ablation Study for PneumoniaMNIST (RadioDINO features)
...
# =============================================================================
# CONFIGURATION (defaults from your PneumoniaMNIST script)
# =============================================================================
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
RADIODINO_MODEL = "hf_hub:Snarcy/RadioDino-s16"
K = 2
ACTIV = "ELU"                    # your current activation
ALPHA = 0.92                     # graph sparsification threshold
CUT = 0                          # 0 = cut loss, 1 = modularity loss
TAU = 0.1                        # contrastive temperature
BETA = 0.3                       # JSD balance (online vs cross)
EMA_DECAY = 0.9                  # target network momentum
LAMBDA_CON = 1.0                 # contrastive loss weight
NUM_EPOCHS_FULL = 2000           # full epochs (your original)
NUM_EPOCHS_ABLATION = 2000       # fewer epochs for faster ablation
T_STRUCT = 1.0                   # temperature for structural loss
C_ENTROPY = 0.05                 # entropy regularisation coefficient
N_EVAL_PASSES = 30
NUM_ARMA_LAYERS = 1              # your best depth (from earlier runs)

# Ablation settings
NUM_SEEDS = 10                    # number of random seeds per config (increase for robustness)
N_EVAL_PASSES_FAST = 30          # full MC passes

# Parameter grids – adjust ranges based on preliminary results
ABLATION_GRIDS = {
    "ACTIV": ["RELU", "ELU", "SELU", "GELU"],
    "NUM_ARMA_LAYERS": [1, 2, 3],
    "LAMBDA_CON": [0.0, 0.5, 1.0, 2.0, 5.0],
    "C_ENTROPY": [0.0, 0.05, 0.1, 0.2, 0.5],
    "T_STRUCT": [0.5, 1.0, 1.5, 2.0],
    "EMA_DECAY": [0.9, 0.99, 0.999],
    "TAU": [0.05, 0.1, 0.2, 0.3],
    "BETA": [0.1, 0.3, 0.5, 0.7],
    "ALPHA": [0.85, 0.90, 0.92, 0.95],
    "CUT": [0, 1],               # 0 = cut loss, 1 = modularity loss
}

# =============================================================================
# DATA LOADING – PneumoniaMNIST + RadioDINO features (once)
# =============================================================================
print("Loading PneumoniaMNIST data and extracting RadioDINO features (once) ...")
data_npz = np.load('/content/drive/MyDrive/TejaswiAbburi_va797/Dataset/Medmnist_data/pneumoniamnist_224.npz', allow_pickle=True)
...
# Subsample: up to 2000 per class (as in your original)
dataset = TensorDataset(X_img, y_img)
class0_indices = [i for i in range(len(y_img)) if y_img[i] == 0]  # Normal
class1_indices = [i for i in range(len(y_img)) if y_img[i] == 1]  # Pneumonia
random.seed(42)
...
# Extract RadioDINO features (once, globally)
radiodino = timm.create_model(RADIODINO_MODEL, pretrained=True)
radiodino.eval().to(DEVICE)
normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                 std=[0.229, 0.224, 0.225])
...
# =============================================================================
# GRAPH UTILITIES (recomputed for each ALPHA value)
# =============================================================================
...
class ARMAEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, device, activ="ELU",
                 num_stacks=1, num_layers=1, num_arma_layers=1):
        super().__init__()
        self.device = device
        self.act = ACTIVATIONS.get(activ, F.elu)
        def _arma(i, o):
...
def train_and_evaluate(config, seeds=NUM_SEEDS, epochs=NUM_EPOCHS_ABLATION, n_eval=N_EVAL_PASSES_FAST):
...
def run_ablation():
    ...
    baseline = train_and_evaluate(default_config, seeds=NUM_SEEDS, epochs=NUM_EPOCHS_ABLATION)
    ...
    for param, values in ABLATION_GRIDS.items():
        for val in values:
            ...
            config[param] = val
            res = train_and_evaluate(config, seeds=NUM_SEEDS, epochs=NUM_EPOCHS_ABLATION)
    ...
    csv_path = "arma_ablation_results_pneumonia.csv"
    df.to_csv(csv_path, index=False)
    print(f"\nAblation completed. Results saved to {csv_path}")
    # Print a summary (rounded)

...
# ── Subsample: up to 1000 per class (same as ARMA pipeline) ──────────────────
...
print(f"Malignant (class 0): {np.sum(y_labels==0)}, Normal (class 1): {np.sum(y_labels==1)}")
...
print(f"    Malignant (n={(y_labels==0).sum()})      {cm_diag[0,0]:>6}       {cm_diag[0,1]:>6}")
print(f"    Normal    (n={(y_labels==1).sum()})      {cm_diag[1,0]:>6}       {cm_diag[1,1]:>6}")

pct_malig_in_c0 = (y_labels[cluster0_mask] == 0).mean() * 100
pct_norm_in_c1  = (y_labels[cluster1_mask] == 1).mean() * 100
...
paper_results = {
    ...
    "Value": [
        ...
        f"{acc_final:.4f}",
        f"{prec_vals[0]:.4f}",   # use first run's weighted precision (seed 0)
        f"{rec_vals[0]:.4f}",
        f"{f1_vals[0]:.4f}",
...
df_paper.to_csv("kmeans_radiodino_breastmnist_summary.csv", index=False)
print("\n  ✓ Saved to: kmeans_radiodino_breastmnist_summary.csv")

# Save per-run results
df_results = pd.DataFrame(results_records)
df_results.to_csv("kmeans_radiodino_breastmnist_per_run.csv", index=False)
print("  ✓ Saved per-run results to: kmeans_radiodino_breastmnist_per_run.csv")

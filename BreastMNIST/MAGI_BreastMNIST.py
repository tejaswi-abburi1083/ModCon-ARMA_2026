...
# =============================================================================
print(f"\n{'═'*72}")
print(f"  MAGI (faithful) + K‑Means — BreastMNIST (α={ALPHA})")
...
df_unc.to_csv("magi_breastmnist_uncertainty_run1.csv", index=False, float_format="%.6f")
print(f"\n  CSV → magi_breastmnist_uncertainty_run1.csv")
...
sep100 = "─" * 100
print(f"\n{sep100}")
print(f"  {N_RUNS}-RUN SUMMARY (mean ± std) — MAGI (BreastMNIST, α={ALPHA})")
print(f"{sep100}")
...
print(f"  {sep100}")
count_labels = [
    ("TN (Malig→C0)", "Malignant→Cluster0"),
    ("FP (Malig→C1)", "Malignant→Cluster1"),
    ("FN (Norm→C0)",  "Normal→Cluster0"),
    ("TP (Norm→C1)",  "Normal→Cluster1"),
]
...
print_clustering_summary(seed_clustering_records, depth_label="MAGI GCN — BreastMNIST (RadioDINO)")
...
df_summary.to_csv("magi_breastmnist_summary.csv", index=False, float_format="%.6f")
print(f"\n  ✓ Saved summary → magi_breastmnist_summary.csv")

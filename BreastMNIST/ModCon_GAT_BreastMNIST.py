...
HIDDEN_DIM = 256
GAT_HEADS = 4            # number of attention heads per layer
GAT_DROPOUT = 0.2        # dropout inside GATConv
...
# ─── GAT ENCODER (multi-head attention) ───────────────────────────────────────
...
class GATEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, activ="RELU",
                 num_layers=3, heads=2, dropout=0.2):
        super().__init__()
        ...
        # Project from concatenated multi-head output to final hidden_dim
...
class GATModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_clusters, device, activ,
                 ...
                 num_gat_layers=3, gat_heads=2, gat_dropout=0.2, T_struct=2.0):
        super().__init__()
        self.device        = device
...
def print_clustering_summary(all_records, depth_label="3 GAT layers"):
    sep = "─" * 90
    print(f"\n{sep}")
...
# ─── MAD METRICS (now with GAT layer names) ───────────────────────────────────
...
def run_once(seed_offset=0, verbose=False, return_probs=False,
    ...
    # Also return unweighted for backwards compatibility
    metrics_unw = (acc_w,
...
print(f"\n{SEP}")
print(f"  PART A — FIRST RUN ({NUM_GAT_LAYERS} GAT layers) – BreastMNIST (RadioDINO {RADIODINO_MODEL})")
...
df_unc0.to_csv("cluster_uncertainty_part_a_gat.csv", index=False, float_format="%.6f")
print(f"\n  CSV → cluster_uncertainty_part_a_gat.csv  ({len(df_unc0)} subjects)")
...
clustering_results_A = evaluate_clustering_from_mc(
    ...
    prefix="Part A — 3-layer GAT, seed 0"
)

print(f"\n{SEP}\n  PART B — PER-LAYER OVER-SMOOTHING ANALYSIS\n{SEP}")
layer_similarity_analysis(model3, features_np, edge_index_np, y, W0,
                          prefix=f"{NUM_GAT_LAYERS}-layer GAT (BreastMNIST / RadioDINO)")

print(f"\n{SEP}\n  PART C — DEPTH ABLATION  (1 / 2 / 3 GAT layers)\n{SEP}")
ablation_results  = {}
trained_models    = {}
...
for n_layers in [1, 2, 3]:
    ...
    for seed in range(3):
        ...
        # FIXED: unpack correctly (only model, ignore metrics)
...
print(f"\n  Over-smoothing ({NUM_GAT_LAYERS} layers):")
print(f"    MADGap_{NUM_GAT_LAYERS}layers={mad_depth[3][1]:.4f}  vs  "
      f"MADGap_input={mad_depth[3][0]:.4f}")
if mad_depth[3][1] < mad_depth[3][0] * 0.5:
    print("    ⚠ OVER-SMOOTHING detected")
else:
    print("    ✓ Gap preserved – GAT attention preserves local structure")

# ─── PART E — 10-SEED EVALUATION (weighted metrics) ───────────────────────────
print(f"\n{SEP}\n  PART E — 10-SEED EVALUATION ({NUM_GAT_LAYERS} GAT layers, BreastMNIST)\n{SEP}")
...
print(f"  10-RUN SUMMARY  (mean ± std)              "
      f"[{NUM_GAT_LAYERS} GAT layers, BreastMNIST / RadioDINO]")
...
print(f"\n  ── F.1  10-SEED EVALUATION ({NUM_GAT_LAYERS} GAT layers) ──")
print_clustering_summary(seed_clustering_records,
                         depth_label=f"{NUM_GAT_LAYERS} GAT layers — 10 seeds (BreastMNIST, RadioDINO)")

print(f"\n  ── F.2  DEPTH ABLATION (3 seeds per depth) ──")
for n_layers in [1, 2, 3]:
    print_clustering_summary(
        ablation_clustering[n_layers],
        depth_label=(f"{n_layers} GAT layer"
...
print(f"    structure in breast ultrasound images. The GAT graph network")
...
df_paper.to_csv("cluster_breastmnist_characterization_gat.csv", index=False)
print(f"\n  ✓ Saved to: cluster_breastmnist_characterization_gat.csv")
df_unc0.to_csv("cluster_uncertainty_breastmnist_gat.csv", index=False,
               float_format="%.6f")
print(f"  ✓ Saved uncertainty to: cluster_uncertainty_breastmnist_gat.csv")

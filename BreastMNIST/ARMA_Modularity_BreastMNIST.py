...
# ═══════════════════════════════════════════════════════════════════════════
#  MODEL DEFINITIONS (simplified ARMA, no contrastive loss)
# ═══════════════════════════════════════════════════════════════════════════
class MLP(nn.Module):
...
last_true_labels = None   # for detailed report

print("═" * 72)
print("  ARMA (modularity only) with MC uncertainty – BreastMNIST (RadioDINO)")
...
print("\n" + "═" * 72)
print("  10-RUN SUMMARY (mean ± std)                         [ARMA modularity only, BreastMNIST]")
print("  NOTE: Diagnostic labels used only for post-hoc external validation.")
print("═" * 72)
...
if last_logits_stack is not None:
    ...
    df.to_csv("arma_modularity_breastmnist_uncertainty_last_run.csv", index=False, float_format="%.6f")
    print(f"\n  ✓ Saved per-subject uncertainty to arma_modularity_breastmnist_uncertainty_last_run.csv")
...
df_all.to_csv("arma_modularity_breastmnist_10run_weighted_summary.csv", index=False, float_format="%.6f")
print(f"\n  ✓ Saved 10‑run weighted summary → arma_modularity_breastmnist_10run_weighted_summary.csv")
